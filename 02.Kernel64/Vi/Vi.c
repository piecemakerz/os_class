...
static void CommandMode(char key){
    if(ESC_FLAG){
        ...
        if( commandLine->characters > 0 ){
            ...
            for( int i = 0 ; i < 1 ; i++ ){
                int iCommandLength = kStrLen( CommandList[ i ].pcCommand );
                // 커맨드의 길이와 내용이 완전히 일치하는지 검사
                if( ( iCommandLength == commandLine->curCharPos ) &&
                    ( kMemCmp( CommandList[ i ].pcCommand,  commandLine->characters ,
                            commandLine->curCharPos ) == 0 ) )
                {
                    CommandList[ i ].pfFunction( commandLine->characters + commandLine->curCharPos + 1 );
                    break;
                }
            }
        }
